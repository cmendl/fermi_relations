...
def test_double_factorization():
    ...
    # check symmetries
    assert np.allclose(vint.transpose(1, 0, 2, 3), vint)
    assert np.allclose(vint.transpose(0, 1, 3, 2), vint)
    assert np.allclose(vint.transpose(2, 3, 0, 1), vint)
    vmat = vint.reshape((nmodes**2, nmodes**2))
    assert np.allclose(vmat.T, vmat)

    lmat = _truncated_sqrtm(vmat)
    assert np.isrealobj(lmat)
    assert np.allclose(lmat @ lmat.T, vmat)
    ltensor = lmat.reshape((nmodes, nmodes, -1))
    # symmetry must be preserved
    assert np.allclose(ltensor.transpose((1, 0, 2)), ltensor)
    ...
    # compare
    assert spla.norm(vop_squares - vop_ref) < 1e-14
    ...
    for l in range(ltensor.shape[-1]):
        ...
        for i in range(len(eig)):
            for j in range(len(eig)):
                ...
                assert spla.norm(fr.comm(nlist[i], nlist[j])) < 1e-14
                vop_number += 0.5 * eig[i] * eig[j] * (nlist[i] @ nlist[j])
    # compare
    assert spla.norm(vop_number - vop_ref) < 1e-14
