...
def test_expectation_value():
    ...
    # compare
    assert abs(avr - avr_ref) < 1e-14


def test_representability_conditions():
    """
    ...
    assert all(0 <= np.linalg.eigvalsh(q))
    ...
    assert all(0 <= np.linalg.eigvalsh(g))
    ...
    t1_ref = np.array([[np.vdot(psi,
                      ...
                      + np.vdot(psi,
                       ...
                       for j in idxtriples]
                       for i in idxtriples])
    assert np.allclose(t1, t1_ref)
    # 't1' must be Hermitian
    assert np.allclose(t1, t1.conj().T)
    # eigenvalues must be non-negative, i.e., 't1' must be positive semidefinite
    assert all(0 <= np.linalg.eigvalsh(t1))
    ...
    t2_ref = np.array([[np.vdot(psi,
                      ...
                      + np.vdot(psi,
                       ...
                       for j in idxtriples]
                       for i in idxtriples])
    assert np.allclose(t2, t2_ref)
    # 't2' must be Hermitian
    assert np.allclose(t2, t2.conj().T)
    # eigenvalues must be non-negative, i.e., 't2' must be positive semidefinite
    assert all(0 <= np.linalg.eigvalsh(t2))
